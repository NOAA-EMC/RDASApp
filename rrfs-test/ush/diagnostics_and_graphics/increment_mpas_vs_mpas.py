...
def plot_mpas_increments(mpas_inc1, mpas_inc2, mlons, mlats, variable, figname, level, clevmax, ctl_name, exp_name):
    """
    Plot MPAS increments from two experiments side by side at a specified level.

    Args:
        mpas_inc1 (ndarray): Increment data for experiment 1 (control).
        mpas_inc2 (ndarray): Increment data for experiment 2 (new experiment).
        ...
        ctl_name (str): Name of the control experiment.
    ...
    # Plot increment for experiment 1 (control)
    c1 = m1.tricontourf(triang, mpas_inc1, clevs, cmap=cm, extend='both')
    m1.set_title(f"{ctl_name}", fontsize=9)

    # Plot increment for experiment 2 (new experiment)
    c2 = m2.tricontourf(triang, mpas_inc2, clevs, cmap=cm, extend='both')
    m2.set_title(f"{exp_name}", fontsize=9)
    ...
    subtitle2 = f"max: {np.around(np.max(mpas_inc2), 4)}\nmin: {np.around(np.min(mpas_inc2), 4)}"
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m2b', '--mpas2_bkg', type=str, required=True, help='MPAS background file for experiment 2 (new experiment)')
    parser.add_argument('-m2a', '--mpas2_ana', type=str, required=True, help='MPAS analysis file for experiment 2 (new experiment)')
    ...
    mpas2_bkg = args.mpas2_bkg
    mpas2_ana = args.mpas2_ana
    ...
    mpas_inc2 = compute_mpas_increment(mpas2_bkg, mpas2_ana, variable, level)

    # Plot with experiment names and specified level
    plot_mpas_increments(mpas_inc1, mpas_inc2, mlons, mlats, variable, figname, level, clevmax=5.0, ctl_name=ctl_name, exp_name=exp_name)
