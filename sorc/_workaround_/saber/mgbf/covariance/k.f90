  ...
  !cltothink                  if(.not.self%intstate(iscale,ivargrp)%l_for_localization )then 
   ...
   !cltorg                 varvlev_index(isize,1)= varvlev_index(isize-1,1)+nz3d
             ...
             do ivargrp=1,nvargrp
                ...
                write(6,*)'thinkdeb888 ivargrp ',ivargrp,' ',nlev_vargrp(ivargrp),' nzloc',nzloc
                call flush(6)
...
!clt                work_mgbf2(ii:ii+nlev_vargrp(ivargrp)-1,:,:)=vargrp_work_mgbf2(:,:,:)
                work_mgbf2(ii:ii+nlev_vargrp(ivargrp)-1,:,:)=vargrp_work_mgbf2(:,:,:)
!                write(6,*)'thinkdeb888 2 ivargrp ',ivargrp,' ',nlev_vargrp(ivargrp),' nzloc',nzloc
                call flush(6)
             ...
             if(.not. self%intstate(jscale,ivargrp0)%l_for_localization ) then   !clthinkdebxxx
                ...
                write(6,*)'thinkdeb888 3 '
                call flush(6)
               work_mgbf=work_mgbf2
                write(6,*)'thinkdeb888 4 '
                call flush(6)
             ...
             enddo
                write(6,*)'thinkdeb888 5 '
                call flush(6)
                ilev=1
                     n_owned_size=0
             do isize=1,fields%size()
     
                write(6,*)'thinkdeb888 5.1 isize ',isize
                call flush(6)
                afield=fields%field(isize)  !clttodo
                fs= afield%functionspace()  !cltthinkfore debug
                n_owned_size= fs%size_owned() !clt for debug
                ...
                write(6,*)'thinkdeb888 5 2 ',nz, ' ',lev1,' name is ',afield%name()
                call flush(6)
                  if(nz.gt.1) then 
   !                  if(n_owned_size == 0) then
   !                  do i = 1, size(ghost)
   ...
   !clt                  write(6,*)'thinkdeb2552 n_owned_size ',n_owned_size,' ','total size is  ' ,size(ptr_2d,2) 
                write(6,*)'thinkdeb888 5 .3 ',n_owned_size
                call flush(6)
                         if(n_owned_size >0 ) then 
                ...
                write(6,*)'thinkdeb888 5 .3 b',n_owned_size,' localization is ',self%intstate(1,1)%l_for_localization
                call flush(6)
                ...
                write(6,*)'thinkdeb888 10 lev1',lev1
                call flush(6)
                                if(n_owned_size >0 ) then 
                ...
                write(6,*)'thinkdeb888 10.2 '
                call flush(6)
                             endif
                     else  !not localization
                write(6,*)'thinkdeb888 5.4 ',n_owned_size,' lev',lev1
                call flush(6)
                          if(n_owned_size >0 ) then 
                             ptr_2d(1,1:n_owned_size)=work2d_mgbf(lev1,:)!if nz=1, only the first level is used (like for surface pressure) 
                          else 
                ...
                write(6,*)'thinkdeb888 5.5 ',n_owned_size,' lev',lev1
                call flush(6)
                       
                     endif  ! for localization
                  endif  ! if nz >1 or =1
   ...
   !clt               ptr_3d=work2d_mgbf(ilev:ilev+nz-1,:) 
              ...
              enddo
                write(6,*)'thinkdeb888 5.6 '
                call flush(6)

             call etim(mg_postprocess_time)


                write(6,*)'thinkdeb888 20 '
                call flush(6)
             ...
             deallocate( varvlev_index)
                write(6,*)'thinkdeb888 20.2 '
                call flush(6)
 !clt       enddo   !for iscale
          call etim(mg_multiply_time)
                write(6,*)'thinkdeb888 20.3 '
                call flush(6)
        deallocate(nlev_vargrp)
                write(6,*)'thinkdeb888 20.4 '
                call flush(6)
